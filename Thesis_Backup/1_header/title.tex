\begin{titlepage}
%\newgeometry{left=20mm,right=20mm,top=2.5cm,bottom=2cm}


\thispagestyle{empty}
\setlength\headheight{0pt} 
\begin{center}

\begin{center}
\includegraphics[width=0.25\linewidth]{img/uwa.PNG}            
\end{center}	
% {\small School of Electrical, Electronic \& Computer Engineering}\\[0.2em]
% {\small Faculty of Engineering, Computing \& Mathematics}\\[0.2em]
% {\small The University of Western Australia}\\[2.5em]

        \vspace{0.25cm}
        {\scshape\LARGE University of Western Australia \par}
        \vspace{0.25cm}
        {\scshape\Large Honours Project\par}
        \vspace{0.5cm}

        {\Large\bfseries Stability in Reinforcement Learning for Query Augmentation: Hyperparameter and Model Size Effects in PPO and GRPO\par}
        
        \vspace{0.5cm}
        {\Large\itshape Daniel Yitian Su\par}
        School of Physics Mathematics and Computing
        \vspace{0.25cm}

\vspace{1cm}
Supervised by\par
Assoc Prof.~Wei Liu Dr.~Daochang Liu  \\
School of Physics Mathematics and Computing\par
\vspace{1.5cm}
\large
\today

\end{center}

\clearpage
\restoregeometry
\end{titlepage}
